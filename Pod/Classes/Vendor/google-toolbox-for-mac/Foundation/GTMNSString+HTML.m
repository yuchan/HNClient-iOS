...
- (NSString *)gtm_stringByEscapingHTMLUsingTable:(HTMLEscapeMap*)table 
  ...
  for (NSUInteger i = 0; i < length; ++i) {
    ...
    if (val || (escapeUnicode && buffer[i] > 127)) {
      ...
      if (val) {
          NSString *escapeSeq = [[NSString alloc] initWithCString:val->escapeSequence encoding:NSUTF8StringEncoding];
        [finalString appendString:escapeSeq];
      }
      else {
    ...
    if (length > 3 && length < 11) {
      if ([escapeString characterAtIndex:1] == '#') {
        ...
        for (unsigned i = 0; i < sizeof(gAsciiHTMLEscapeMap) / sizeof(HTMLEscapeMap); ++i) {
            NSString *escapeSeq = [[NSString alloc] initWithCString:gAsciiHTMLEscapeMap[i].escapeSequence encoding:NSUTF8StringEncoding];
          if ([escapeString isEqualToString:escapeSeq]) {
